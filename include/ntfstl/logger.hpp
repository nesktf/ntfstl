...
enum class log_level : uint8_t { error = 0, warning, info, debug, verbose };

class logger {
public:
  static inline void set_level(log_level new_level) { _log_level = new_level; }
  ...
  static inline void _log(log_level level, const std::string& prefix, const std::string& str_color,
    ...
    fmt::print("[{:%H:%M:%S}.{:03d}]\033{}[{}]\033[0m{}\n", *time_tm, (int)ms, str_color, prefix,
               log);
#else
  ...
  static inline void verbose(fmt::format_string<Args...> format, Args&&... args) {
    _log(log_level::verbose, "VERBOSE", NTF_LOG_VERBOSE_COLOR, format,
