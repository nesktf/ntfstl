...
public:
  ...
  template<typename U>
  constexpr optional_data(U&& obj) noexcept(meta::is_nothrow_forward_constructible<U>) :
...
public:
  constexpr optional_data() noexcept(std::is_nothrow_copy_constructible_v<T>) :
      _value(optional_null<T>::value) {}

  constexpr optional_data(nullopt_t) noexcept(std::is_nothrow_copy_constructible_v<T>) :
      _value(optional_null<T>::value) {}
...
protected:
  constexpr void destroy() noexcept { _value = optional_null<T>::value; }
  ...
  template<typename U, typename... Args>
  constexpr void construct(std::initializer_list<U> il, Args&&... args) noexcept(
    std::is_nothrow_constructible_v<T, std::initializer_list<U>, Args...>) {
    _value.~T();
    new (std::addressof(_value)) T(il, std::forward<Args>(args)...);
  }
