...
public:
  ...
  bool operator!=(const fhashmap_forward_it& other) const { return !(other == *this); }
...
private:
  explicit fhashmap_forward_it(MapT* map, size_t idx) : _map{map}, _idx{idx} { _next_valid(); }
...
struct fixed_hashmap_ops : private HashT, private EqualsT {
  fixed_hashmap_ops() : HashT{}, EqualsT{} {}

  fixed_hashmap_ops(const HashT& hash, const EqualsT& equals) : HashT{hash}, EqualsT{equals} {}

  bool _equals(const K& a, const K& b) const { return EqualsT::operator()(a, b); }

  EqualsT& _key_eq() { return static_cast<EqualsT&>(*this); }

  const EqualsT& _key_eq() const { return static_cast<const EqualsT&>(*this); }

  size_t _hash(const K& x) const { return HashT::operator()(x); }
...
struct fixed_hashmap_del : private Del, private meta::rebind_deleter_t<Del, fhashmap_flag> {
private:
  using value_base_t = Del;
  using flag_base_t = meta::rebind_deleter_t<Del, fhashmap_flag>;

public:
  fixed_hashmap_del() : value_base_t{}, flag_base_t{} {}
  ...
  fixed_hashmap_del(const Del& del) : value_base_t{del}, flag_base_t{del} {}

  void _destroy(std::pair<const K, T>* values, size_t value_count, fhashmap_flag* flags,
                size_t flag_count) {
...
template<
  typename K, typename T, typename HashT = std::hash<K>, typename EqualsT = std::equal_to<K>,
...
public:
  fixed_hashmap(value_type* values, flags_type* flags, size_t cap, size_t used) :
      del_base_t{}, ops_base_t{}, _values{values}, _flags{flags}, _used{used}, _cap{cap} {}

  fixed_hashmap(value_type* values, flags_type* flags, size_t cap, size_t used, const DelT& del,
                const HashT& hash, const EqualsT& eqs) :
      del_base_t{del}, ops_base_t{hash, eqs}, _values{values}, _flags{flags}, _used{used},
      _cap{cap} {}

  template<typename Alloc = std::allocator<value_type>, typename Hash = HashT,
           typename Equals = EqualsT>
  requires(meta::allocator_type<std::decay_t<Alloc>, value_type>)
  fixed_hashmap(size_type size, Alloc&& alloc = {}, const Hash& hash = {},
                const Equals& eqs = {}) :
      del_base_t{in_place, alloc}, ops_base_t{hash, eqs}, _values{nullptr}, _flags{nullptr},
      _used{0u}, _cap{size} {
  ...
  template<typename Alloc = std::allocator<value_type>, typename Hash = HashT,
           typename Equals = EqualsT>
  requires(meta::allocator_type<std::decay_t<Alloc>, value_type>)
  fixed_hashmap(std::initializer_list<value_type> il, Alloc&& alloc = {}, const Hash& hash = {},
                const Equals& eqs = {}) :
  ...
  fixed_hashmap(fixed_hashmap&& other) noexcept :
      del_base_t{static_cast<del_base_t&&>(other)}, ops_base_t{static_cast<ops_base_t&&>(other)},
...
public:
  template<typename Alloc = std::allocator<value_type>, typename Hash = HashT,
           typename Equals = EqualsT>
  requires(meta::allocator_type<std::decay_t<Alloc>, value_type>)
  static alloc_ret<Alloc, Hash, Equals> from_size(size_type size, Alloc&& alloc = {},
  ...
  template<typename Alloc = std::allocator<value_type>, typename Hash = HashT,
           typename Equals = EqualsT>
  requires(meta::allocator_type<std::decay_t<Alloc>, value_type> && std::copy_constructible<K> &&
           std::copy_constructible<T>)
  static alloc_ret<Alloc, Hash, Equals> from_initializer(std::initializer_list<value_type> il,
                                                         Alloc&& alloc = {}, const Hash& hash = {},
                                                         const Equals& eqs = {}) {
...
public:
  ...
  mapped_type& operator[](const key_type& key) noexcept(NTF_NOEXCEPT) { return at(key); }
